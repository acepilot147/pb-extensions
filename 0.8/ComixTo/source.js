...
var _Sources = (() => {
  ...
  function getSboxInverseRuntime() {
    if (SBOX_INVERSE_RUNTIME === null) {
      SBOX_INVERSE_RUNTIME = SBOX_INVERSE_STAGES.map((stage) => {
        ...
        const inverseTable = new Array(256).fill(0);
  ...
  function applySboxCbcInverseStage(data, stage) {
    const out = new Array(data.length);
    let prev = stage.iv & 255;
    for (let i = 0; i < data.length; i++) {
      const cipherByte = data[i] & 255;
      const idx = stage.inverseTable[cipherByte] & 255;
      out[i] = (idx ^ stage.key[i % stage.key.length] ^ prev) & 255;
      prev = cipherByte;
    }
    return out;
  }
  ...
  function applySboxCbcStage(data, stage) {
    const table = b64Decode2(stage.tableB64);
    const key = b64Decode2(stage.keyB64);
    const out = new Array(data.length);
    let prev = stage.iv & 255;
    for (let i = 0; i < data.length; i++) {
  ...
  function applyMutationStage(data, stage) {
    const ops = b64Decode3(stage.opsB64);
  ...
  function applySboxCbcStage2(data, stage) {
    const table = b64Decode3(stage.tableB64);
    const key = b64Decode3(stage.keyB64);
    const out = new Array(data.length);
    let prev = stage.iv & 255;
    for (let i = 0; i < data.length; i++) {
      const idx = (data[i] & 255 ^ key[i % key.length] ^ prev) & 255;
  ...
  function applySboxCbcInverseStage2(data, stage) {
    const table = b64Decode3(stage.tableB64);
    const key = b64Decode3(stage.keyB64);
    const inverseTable = new Array(256).fill(0);
    for (let i = 0; i < 256; i++) inverseTable[table[i]] = i;
    ...
    for (let i = 0; i < data.length; i++) {
      const cipherByte = data[i] & 255;
      const idx = inverseTable[cipherByte] & 255;
      out[i] = (idx ^ key[i % key.length] ^ prev) & 255;
      prev = cipherByte;
    }
    return out;
  }
  ...
  function remoteGenerateHash(rawPath, constants) {
    ...
    if (constants.signer.pipelineOrder === "sbox-cbc") {
      let data2 = bytesFromString2(path);
      for (const stage of constants.signer.sboxCbcStages ?? []) data2 = applySboxCbcStage2(data2, stage);
      return b64UrlEncode2(data2);
    }
    ...
    for (let i = 0; i < rc4Keys.length; i++) {
      if (constants.signer.pipelineOrder === "rc4-then-insert") {
        data = rc4(b64Decode3(rc4Keys[i]), data);
  ...
  function remoteDecryptComixPayload(payload, headers, constants) {
    ...
    if (constants.decrypt.pipelineOrder === "sbox-cbc-inverse") {
      for (const stage of constants.decrypt.sboxInverseStages ?? []) data = applySboxCbcInverseStage2(data, stage);
    } else {
      for (let i = 0; i < constants.decrypt.rc4Keys.length; i++) {
        if (constants.decrypt.pipelineOrder === "mutation-then-rc4") {
          ...
          data = rc4(b64Decode3(constants.decrypt.rc4Keys[i]), data);
        } else {
          data = rc4(b64Decode3(constants.decrypt.rc4Keys[i]), data);
          data = applyMutationStage(data, constants.decrypt.mutationStages[i]);
        }
