...
// Reassemble decoded raw RGB/RGBA pixels in-place from scrambled order to
// clean order. The caller must have already decoded the WebP to a raw pixel
// buffer (RGB or RGBA, row-major, no padding). Returns a new buffer; does
// not mutate the input.
//
// Provided as a utility for environments that do have a pixel buffer to work
// with (Node-side relay, Web Worker with createImageBitmap, etc.). Paperback's
// extension VM does not have an image decoder, so this is unused in-extension.
export function descramblePixels(
  src: Uint8Array,
  width: number,
  height: number,
  channels: number,
  ...
  const { seed, cols, rows } = params;
  const tileCount = cols * rows;
  if (width % cols !== 0 || height % rows !== 0) {
    throw new Error(
      `image dimensions ${width}x${height} not divisible by grid ${cols}x${rows}`,
    );
  }
  const expected = width * height * channels;
  if (src.length !== expected) {
    throw new Error(`src length ${src.length} != expected ${expected}`);
  }
  const lookup = computeDescrambleLookup(seed, tileCount);
  const tw = width / cols;
  const th = height / rows;
  const out = new Uint8Array(expected);
  const rowBytes = tw * channels;
  for (let cleanIdx = 0; cleanIdx < tileCount; cleanIdx++) {
    const srcIdx = lookup[cleanIdx]!;
    const cr = (cleanIdx / cols) | 0;
    const cc = cleanIdx % cols;
    const sr = (srcIdx / cols) | 0;
    const sc = srcIdx % cols;
    for (let y = 0; y < th; y++) {
      const srcOff = ((sr * th + y) * width + sc * tw) * channels;
      const dstOff = ((cr * th + y) * width + cc * tw) * channels;
