...
function validateCompactPayload(payload) {
  ...
  for (const stage of payload.decrypt.sboxInverseStages || []) {
    if (decodeB64(stage.tableB64).length !== 256) {
      throw new Error("Decrypt sbox inverse table length mismatch");
    }
    if (decodeB64(stage.keyB64).length === 0) {
      throw new Error("Decrypt sbox inverse key empty");
    }
  }
}
...
function buildPayload() {
  ...
  assertOrder(signerOrder, ["rc4-then-insert", "insert-then-rc4", "sbox-cbc"], "signer");
  assertOrder(decryptOrder, ["mutation-then-rc4", "rc4-then-mutation", "sbox-cbc-inverse"], "decrypt");

  const signerSboxStages = signerOrder === "sbox-cbc" ? parseGeneratedArray(signerSource, "SBOX_CBC_STAGES") : [];
  const signerInsertStages = signerOrder === "sbox-cbc" ? [] : parseGeneratedArray(signerSource, "INSERT_STAGES").map(compactInsertStage);
  const decryptIsSbox = decryptOrder === "sbox-cbc-inverse";
  const decryptMutationStages = decryptIsSbox ? [] : parseGeneratedArray(decryptSource, "MUTATION_STAGES").map(compactMutationStage);
  const decryptSboxStages = decryptIsSbox ? parseGeneratedArray(decryptSource, "SBOX_INVERSE_STAGES") : [];
  ...
  const payload = {
    ...
    signer: {
      ...
      roundSteps: signerOrder === "sbox-cbc" ? ["sbox-cbc"] : signerOrder.split("-then-"),
      rc4Keys: signerOrder === "sbox-cbc" ? [] : parseGeneratedArray(signerSource, "RC4_KEYS"),
      insertStages: signerInsertStages,
      sboxCbcStages: signerSboxStages
    },
    decrypt: {
      pipelineOrder: decryptOrder,
      roundSteps: decryptIsSbox ? ["sbox-cbc-inverse"] : decryptOrder.split("-then-"),
      rc4Keys: decryptIsSbox ? [] : parseGeneratedArray(decryptSource, "RC4_KEYS"),
      mutationStages: decryptMutationStages,
      sboxInverseStages: decryptSboxStages
    }
  };
